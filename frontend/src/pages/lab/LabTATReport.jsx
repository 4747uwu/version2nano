...
const LabTATReport = () => {
  ...
  const filteredStudies = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(study => 
        ...
        (study.doctorName || '').toLowerCase().includes(search)
      );
    }
  ...
  const fetchTATData = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get('/lab/tat/report', { params });
  ...
  const exportToExcel = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get('/lab/tat/report/export', { 
        params,
        responseType: 'blob'
      });
      ...
      a.href = url;
      a.download = `Lab_TAT_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 p-2">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-lg font-bold text-gray-900">TAT Performance Report - Lab</h1>
            <p className="text-xs text-gray-600">
              {loading ? (
                <span className="text-orange-600">🔄 Loading studies...</span>
              ) : (
                <>
        ...
        <div className="grid grid-cols-12 gap-1 items-end mb-2">
          ...
          <div className="col-span-1">
            <select
              ...
              className="w-full px-1 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-orange-500"
            >
          ...
          <div className="col-span-4">
            <div className="relative">
              <input
                ...
                className="w-full pl-5 pr-1 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-orange-500"
              />
          ...
          <div className="col-span-2">
            <button
              ...
              disabled={loading}
              className="w-full px-1 py-1 bg-orange-600 text-white rounded text-xs hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                ...
                >
                  ...
                  <td className="border-r border-gray-100 px-1 py-1 whitespace-nowrap">
                    <div className="font-mono text-xs text-gray-700">
                      {formatDateTime(study.studyDate)}
                    </div>
                  </td>
                  <td className="border-r border-gray-100 px-1 py-1 whitespace-nowrap">
                    <div className="font-mono text-xs text-gray-700">
                  ...
                  <td className="border-r border-gray-100 px-1 py-1">
                    <div className="text-gray-700 text-xs truncate max-w-[100px]" title={safeValue(study.doctorName)}>
                      {safeValue(study.doctorName)}
                    </div>
                  </td>
                  <td className="border-r border-gray-100 px-1 py-1 whitespace-nowrap">
                    <div className="font-mono text-xs text-gray-700">
                  ...
                  {/* <td className="border-r border-gray-100 px-1 py-1 text-center whitespace-nowrap">
                    <span className={`inline-flex items-center px-1 py-0.5 text-xs font-medium rounded-full ${getTATStatusColor(study.calculatedTAT)}`}>
                      {safeValue(study.calculatedTAT)} {study.calculatedTAT !== '-' ? 'hrs' : ''}
                    </span>
                  </td> */}
                  <td className="border-r border-gray-100 px-1 py-1 text-center whitespace-nowrap">
